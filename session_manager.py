...
class SessionManager:
    def __init__(self, redis_client: RedisClient, rabbitmq_client: RabbitMQClient):
    ...
    def start_session(self, camera_id: str) -> str:
        ...
        with self.lock:
            session_id = self.redis_client.create_session(camera_id)
            ...
            self.rabbitmq_client.send_session_event(
    ...
    def update_session(self, session_id: str, detections: List[Dict]) -> bool:
        ...
        with self.lock:
            ...
            self.redis_client.update_session(session_id, person_count)
            
            # Продляем TTL сессии
            self.redis_client.extend_session_ttl(session_id)
    ...
    def _close_session_internal(self, session_id: str):
        ...
        if session_id in self.active_sessions:
            ...
            self.redis_client.close_session(session_id)
